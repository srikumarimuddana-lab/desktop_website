 ...
 *
 * Every call is signed (lib/spinr-signing.js) when SPINR_WEB_SIGNING_SECRET is
 * set. That signature is what gets these calls past the backend's App Check
 * gate, and it carries the visitor's IP so the backend rate-limits per
 * visitor rather than per Vercel egress IP.
 */
import { MIN_SECRET_LEN, signingHeaders } from '@/lib/spinr-signing'

/** Base URL of the Spinr backend, e.g. https://api-spinr.spinr.ca/api/v1 */
const RAW_BASE = process.env.SPINR_API_URL || ''

/* Same value as the backend's WEB_CALLER_SIGNING_SECRETS. Unset = calls go
   out unsigned, exactly as before signing existed. */
const SIGNING_SECRET = process.env.SPINR_WEB_SIGNING_SECRET || ''
if (SIGNING_SECRET && SIGNING_SECRET.length < MIN_SECRET_LEN) {
  console.error(
    `[spinr-api] SPINR_WEB_SIGNING_SECRET is shorter than ${MIN_SECRET_LEN} characters and is ignored — ` +
      'backend calls go out unsigned and will be refused by App Check'
  )
}

function signed(method, url, body, clientIp) {
  return signingHeaders({ secret: SIGNING_SECRET, method, url, body, clientIp })
}
...
/*
 * A base URL with no scheme ("api-spinr.spinr.ca") or a stray space is the
 * likeliest way to get this wrong, and `new URL()` throws on both. That throw
 * used to escape buildUrl and take the whole server render down, so a typo in
 * one environment variable read as a broken page rather than as a broken
 * setting. Check it once, loudly, and let every caller degrade instead.
 */
const BASE_OK = (() => {
  ...
  try {
    ...
    console.error(
      `[spinr-api] SPINR_API_URL is set but not a usable URL: ${JSON.stringify(RAW_BASE)} — ` +
        'it needs a scheme, e.g. https://api-spinr.spinr.ca'
    )
    return false
  }
})()
 ...
 */
async function getJson(path, params, { clientIp } = {}) {
  if (!isSpinrApiConfigured() || !BASE_OK) return null
  ...
  try {
    const res = await fetch(url, {
      ...
      headers: { Accept: 'application/json', ...signed('GET', url, '', clientIp) },
 ...
 *          visitorType?: 'rider'|'driver', timeoutMs?: number, clientIp?: string}} opts
 */
export async function askSpinrAssistant({ message, history, visitorType, timeoutMs, clientIp } = {}) {
  if (!isSpinrApiConfigured() || !BASE_OK || !message) return null
  ...
  try {
    const url = buildUrl('/ai/public-chat')
    ...
    const res = await fetch(url, {
      ...
      headers: {
        ...
        ...signed('POST', url, payload, clientIp),
      },
      cache: 'no-store',
      body: payload,
    })
...
async function postJson(path, body, { token, timeoutMs, clientIp } = {}) {
  if (!isSpinrApiConfigured() || !BASE_OK) return { ok: false, status: 0, data: null, reason: 'unconfigured' }
...
export function sendDriverOtp(phone, { clientIp } = {}) {
  return postJson('/auth/send-otp', { phone }, { clientIp })
}
 ...
 */
export function verifyDriverOtp({ phone, code, consentAccepted, clientIp }) {
  return postJson(
    '/auth/verify-otp',
    {
      phone,
 ...
 */
export function registerDriver(token, payload, { clientIp } = {}) {
  return postJson('/drivers/register', payload, { token, clientIp })
}

/** Active top-level service areas (airport zones excluded backend-side). */
export async function fetchServiceAreas({ clientIp } = {}) {
  const rows = await getJson('/service-areas', undefined, { clientIp })
...
export async function fetchVehicleTypes(serviceAreaId, { clientIp } = {}) {
  const rows = await getJson('/vehicle-types', { service_area_id: serviceAreaId }, { clientIp })
  return Array.isArray(rows) ? rows.filter((r) => r && r.id && r.name) : null
}
