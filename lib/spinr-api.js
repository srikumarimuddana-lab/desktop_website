...
/**
 * Budget for a public fare estimate.
 *
 * Longer than a content read because the backend blocks on Google Directions
 * to get the ROAD distance before it prices — pricing on straight-line
 * distance undercharges, so that wait is deliberate on their side (up to 3.5s
 * worst case) and we have to allow for it plus the fare/driver work on top.
 * Still bounded: this runs inside the estimate page's server render, and a
 * visitor waiting on a spinner is worse than an honest "try again".
 */
const ESTIMATE_TIMEOUT_MS = parseInt(process.env.SPINR_ESTIMATE_TIMEOUT_MS || '10000', 10)

/**
 * Real fare quote from the backend's own fare engine.
 *
 * This is the same `compute_ride_estimates` the rider app quotes from, so the
 * price shown here is the price the app would show — surge, area fees, tax and
 * per-vehicle rates included. The website does no fare arithmetic of its own,
 * deliberately: the numbers that used to live in TripEstimate.js were invented
 * and had no relationship to what anyone was actually charged.
 *
 * Returns { ok, status, data } rather than null-on-failure, because the page
 * has to tell these apart: outside the service area is a real answer worth
 * showing, a disabled flag is expected while the feature is dark, and a
 * timeout is worth a retry.
 *
 * @param {{pickup: {lat: number, lng: number}, dropoff: {lat: number, lng: number},
 *          requiresWav?: boolean}} trip
 */
export function fetchPublicEstimate({ pickup, dropoff, requiresWav = false } = {}) {
  if (!pickup || !dropoff) return Promise.resolve({ ok: false, status: 0, data: null, reason: 'incomplete' })
  return postJson(
    '/rides/public-estimate',
    {
      pickup_lat: pickup.lat,
      pickup_lng: pickup.lng,
      dropoff_lat: dropoff.lat,
      dropoff_lng: dropoff.lng,
      requires_wav: Boolean(requiresWav),
    },
